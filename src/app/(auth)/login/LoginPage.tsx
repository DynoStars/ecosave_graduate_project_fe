      ...
      >
        <div className="lg:w-[70%] w-full">
          ...
          <form className="mt-4 space-y-6" onSubmit={handleLogin}>
            <div>
              ...
              <input
                type="email"
                id="email"
                className={`mt-1 w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-light ${emailError ? "border-red-500" : "border-gray-300"}`}
                value={email}
                onChange={(e) => setEmail(e.target.value)}
              />
              {emailError && <p className="text-red-500 text-sm">Email chưa chính xác</p>}
            </div>
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  id="password"
                  className={`mt-1 w-full p-3 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary-light ${passwordError ? "border-red-500" : "border-gray-300"}`}
                  value={password}
              ...
              {passwordError && <p className="text-red-500 text-sm">Mật khẩu chưa chính xác</p>}
            </div>
            <div className="flex items-center justify-between">
              <label className="flex items-center text-gray-600">
                <input type="checkbox" className="mr-2" /> Ghi nhớ tài khoản
              </label>
              <Link href="/forgot-password" className="text-primary-light hover:underline">Quên mật khẩu</Link>
            </div>
            <button
              type="submit"
          ...
          <p className="mt-6 text-center text-gray-600">
            Chưa có tài khoản? <Link href="/register" className="text-primary-light hover:underline">Đăng ký</Link>
          </p>
        </div>
      </motion.div>
      <motion.div
